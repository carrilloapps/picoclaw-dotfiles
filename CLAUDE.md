...
- **Cross-session recall**: Env var `AGENT_CROSS_CHAT_ACCESS` (default `false` in `.env.example`, `true` on this device) authorizes the agent to recall messages across sessions, accounts, and channels via `~/bin/chat-history.sh`. `device-context.sh` renders the resolved policy into AGENT.md on each regeneration. RAG is the owner's own memory; the single-tenant deployment assumption is explicit in the policy section.
- **Orchestration default**: config template now ships all skill/spawn/subagent/workflow/mcp tools `enabled: true`. AGENT.md has an Orchestration Policy section telling the LLM to use every capability, chain tools freely, declare `~/bin/workflow.sh` runs for anything >3 steps, and produce explicit plans on `"dame un plan"` / `"cómo harías X"` with a single confirmation then full execution.
